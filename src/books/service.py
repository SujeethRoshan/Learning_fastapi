# BookService class is going to have all our CRUD logic related to CRUD with the database.
...
class BookService:
    ...
    async def get_all_books(self, session: AsyncSession):
        # give select() and then the model you want to select from. order_by(desc(Book.created_at)): This sorts the results by the created_at column in descending order.
        statement = select(Book).order_by(desc(Book.created_at))
        result = await session.exec(statement)
        # This returns all books in the database as a list of Book instances.
        return result.all()
    ...
    # here we are doing API validation again so that our service is reusable.
    async def create_book(self, book_data: BookCreateModel, session: AsyncSession):
        book_data_dict = book_data.model_dump()
        # Book is your SQLModel/ORM model for the books table.
        # **book_data_dict unpacks the dictionary, so it’s like calling:Book(title="Python 101", author="John")
    ...
    async def update_book(self, book_uid: UUID, update_data: BookUpdateModel, session: AsyncSession):
        statement = select(Book).where(Book.uid == book_uid)
        result = await session.exec(statement)
        # book is a Python object that maps to a row in your books table.
