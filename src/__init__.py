...
# This imports a special Python utility that helps you write asynchronous context managers.Context managers are objects that handle setup and teardown, usually with with statements.
from contextlib import asynccontextmanager
from src.db.main import init_db


# This decorator tells FastAPI to treat this function as a lifecycle handler.
# FastAPI uses this to run code before your app starts serving requests (setup), and after the app is shutting down (teardown).
@asynccontextmanager
...
async def lifespan(app: FastAPI):
   # print(f"Starting up the application...") this is a example of a startup event. and this is how we can create database connection at start of our server.
    ...
    # Once init_db() finishes, it continues to the next line: yield. But while waiting, the event loop is not blocked, so other async tasks can run in the meantime.
    await init_db()
    yield
...
# tags below are Used to group endpoints in Swagger docs under “books”
...
# SQLAlchemy is a popular ORM for Python. SQL model is built for fastapi on top of SQLAlchemy.
...
# pip install asyncpg - for installing async DBAPI.
# put ur environment variables in .env file then we need to access them in our code.
# There are many ways to access those env variable and to be specific with fastapi we can use BaseSettings class from pydantic settings.
